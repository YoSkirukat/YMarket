...
Сервис обрабатывает заказы сам, без нажатия кнопок:

- при старте сервера запускается фоновая синхронизация заказов
  (`src/instrumentation.ts`) — по умолчанию раз в 60 секунд;
- она выдаёт коды по оплаченным цифровым заказам и подтягивает актуальный
  статус из Маркета (в т.ч. `DELIVERED` — его Маркет выставляет сам после
  доставки письма с кодами);
- дополнительно заказы обновляются по вебхуку — это быстрее, но не обязательно.

Интервал задаётся переменной `AUTO_SYNC_INTERVAL_MS` (мс). Значение `0`
выключает фоновую синхронизацию — тогда нужен внешний cron или вебхук.
